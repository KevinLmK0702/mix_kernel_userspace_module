...
- **主界面**：顶部大标题 + **状态大卡**（只有文字：状态 / 当前模式+包名 / 目标与帧率档位 /
  ...
  状态药丸（enable / boost / rtg）、信息行列表（实测帧率 / 提频状态 / 触发次数 / 热控压制 / 外部改频）、
  可折叠的「内核原始状态」与「WALT 调度器」、模式切换（省电/均衡/性能/极速），底部「日志」入口。
- **配置**：每应用参数列表（带图标，可直接改 帧率 / 裕量 / 提频% / 保持ms / RTG 五个字段，
  编辑器占整行宽度、填 `-` = 沿用模式）、添加包名、**排除**（写 `!包名` 黑名单行）、
  从已安装应用挑选（带图标/搜索，已添加的那项显示可点的**移除**）；删除 / 移除都**立即写盘**，
  不必再点顶部「保存列表」；
  页面下方是**快速参数**（立即写 `/proc`）与**全局选项**
  （写入 `fps_boost_ctl.opts`，持久）；导航徽标显示已单独配置的条数。
...
「全局选项」卡片会在打开时从 `fps_boost_ctl.opts` 读回，停留在配置页时由内核 `status` 持续对齐，
